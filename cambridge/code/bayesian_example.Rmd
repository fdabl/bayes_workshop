...
```{r}
gibbs <- function(mu0, w0, v0, sigma0, y, n.iter = 5000) {
  n <- length(y)
  ...
  for (i in 2:n.iter) {
    ...
    mu_post[i] <- rnorm(1, mu1, w1)
    
    S <- sum((y - mu)^2)
    var_post[i] <- 1 / rgamma(1, v1 / 2, S / 2)
  }
  
  cbind(mu_post, var_post)
}
```
...
post0 <- gibbs(50, 2, 4, 10, subset(dat$score, dat$hat == 0))
post1 <- gibbs(50, 2, 4, 10, subset(dat$score, dat$hat == 1))
...
ppsim <- function(sample, n, times = 100) {
...
plot_ppsim <- function(y, post, title = '') {
  times <- 100
  n <- length(y)
  ppost <- ppsim(post, n, times)
  
  yrep <- ppost[, sample(1:times)[1]]
...
```{r, fig.width = 6, fig.height = 6}
ppost1 <- ppsim(post1, 50)
plots1 <- lapply(1:6, function(.x) plot_ppsim(nohat, ppost1))
do.call(grid.arrange, plots0)
```
...
Contrary to the classical *p* value, here we want values close to .5. A simple example using the mean as the test statistic T yields:

```{r}
par(mfrow = c(1, 1))
ppcheck <- function(ppsamples, test) apply(ppsamples, 2, test)

T_mean <- ppcheck(ppost0, mean)
hist(T_mean, breaks = 20, xlab = 'mean creativity score',
     main = paste('posterior predictive p =', mean(T_mean > mean(nohat))))
abline(v = mean(nohat), col = 'red', lwd = 2, lty = 'dashed')
```
